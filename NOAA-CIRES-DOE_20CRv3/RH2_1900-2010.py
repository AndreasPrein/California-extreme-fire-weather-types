...
SaveDir='/glade/campaign/mmm/c3we/prein/NCEP-20C/daymean/RH2/'
...
for yy in range(len(YYYYall)):
    ...
    for me in range(80):
        ...
        SP_file=DataDir+Vars[2]+'/'+Vars[2]+'.'+str(YYYYall[yy])+'_mem'+str(me+1).zfill(3)+'.nc'
...
#             print( '    '+File_fin)
            ...
            ncid.close()
            ncid=Dataset(SP_file, mode='r')
            rgrSP=np.squeeze(ncid.variables[Vars[2]][:])
            ncid.close()
            RH_vals = wrf.rh(rgrQ, rgrSP, rgrT, meta=False)
