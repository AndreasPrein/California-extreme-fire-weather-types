...
def ReadERA5(grWTregion,
    ...
    ncid=Dataset('/glade/work/jaye/fire/U850/U850_2006.nc', mode='r') # open the netcdf file
...
def VarDecomp_4D(DATA,DimensionNames):
    ...
    #Main Terms
    DN=DimensionNames
    V=1./(DATA.shape[0])*np.sum((np.mean(DATA, axis=(1,2,3))-np.mean(DATA))**2)
    E=1./(DATA.shape[1])*np.sum((np.mean(DATA, axis=(0,2,3))-np.mean(DATA))**2)
    D=1./(DATA.shape[2])*np.sum((np.mean(DATA, axis=(0,1,3))-np.mean(DATA))**2)
    S=1./(DATA.shape[3])*np.sum((np.mean(DATA, axis=(0,1,2))-np.mean(DATA))**2)
    # Interaction terms
    VE=1./(DATA.shape[0]*DATA.shape[1])*np.sum((np.mean(DATA, axis=(2,3))-np.mean(DATA, axis=(1,2,3))[:,None]-np.mean(DATA, axis=(0,2,3))[None,:]+np.mean(DATA))**2)
    VD=1./(DATA.shape[0]*DATA.shape[2])*np.sum((np.mean(DATA, axis=(1,3))-np.mean(DATA, axis=(1,2,3))[:,None]-np.mean(DATA, axis=(0,1,3))[None,:]+np.mean(DATA))**2)
    VS=1./(DATA.shape[0]*DATA.shape[3])*np.sum((np.mean(DATA, axis=(1,2))-np.mean(DATA, axis=(1,2,3))[:,None]-np.mean(DATA, axis=(0,1,2))[None,:]+np.mean(DATA))**2)
    ED=1./(DATA.shape[1]*DATA.shape[2])*np.sum((np.mean(DATA, axis=(0,3))-np.mean(DATA, axis=(0,2,3))[:,None]-np.mean(DATA, axis=(0,1,3))[None,:]+np.mean(DATA))**2)
    ES=1./(DATA.shape[1]*DATA.shape[3])*np.sum((np.mean(DATA, axis=(0,2))-np.mean(DATA, axis=(0,2,3))[:,None]-np.mean(DATA, axis=(0,1,2))[None,:]+np.mean(DATA))**2)
    DS=1./(DATA.shape[2]*DATA.shape[3])*np.sum((np.mean(DATA, axis=(0,1))-np.mean(DATA, axis=(0,1,3))[:,None]-np.mean(DATA, axis=(0,1,2))[None,:]+np.mean(DATA))**2)
